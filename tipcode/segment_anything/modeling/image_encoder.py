...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor,fre:torch.Tensor,fre_adapter:torch.nn.Module) -> torch.Tensor:
        #print(x.size())
        x = self.patch_embed(x)  # pre embed: [1, 3, 1024, 1024], post embed: [1, 64, 64, 768]
        #print(x.size())
        #print(fre.size())
        #fre = fre.unsqueeze(1)
        #fre = fre.repeat(1,3,1,1)
        #fre = self.patch_embed(fre)
        #x = x+fre
        #x = torch.cat([x,fre],dim=-1)
        #x = fre_adapter[0].reduce(x)
        #print(fre.size())
        ...
        for i,blk in enumerate(self.blocks):
            x = blk(x)
            # if i==11:
            #     xtt = x.permute(0,3,1,2)
            #     show_feature_channels_colormap(xtt)
            #if i in index:
            #print('1')
            #print(fre_adapter)
            x = fre_adapter[i](x,fre)
        #x = x+res[0]+res[1]+res[2]
        x = self.neck(x.permute(0, 3, 1, 2))  # [b, c, h, w], [1, 256, 64, 64]
        #show_feature_channels_colormap(x)
