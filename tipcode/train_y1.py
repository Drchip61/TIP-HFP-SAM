...
# input = torch.rand(3,3,512,512).cuda()
# a = torch.rand(3,512,512).cuda()
# flops,param = profile(model,(input,a,1,512))
...
train_path = 'train'
cfg = dataset_fre.Config(datapath=train_path, savepath='./saved_model/msnet', mode='train', batch=16, lr=0.05, momen=0.9, decay=5e-4, epoch=50)
data = dataset_fre.Data(cfg)
...
for epoch_num in range(EPOCH):
    ...
    for i_batch,(im1,label0,label2,label1) in enumerate(tqdm.tqdm(train_loader,ncols=60,postfix=show_dict)):  #,edge0,edge1,edge2,edge3
        im1 = im1.cuda().float()
        label0 = label0.cuda()#.long()
        label2 = label2.cuda()#.long()
        fre = label1.cuda()
        #print(fre.size())
        #print(fre.size())
        #_  = label0.unsqueeze(1)
        #label1 = up(_).squeeze(1)
        
        outputs = model(im1,fre,1,512)#[:,:2,:,:]
...
#         break
        ...
        #loss2 = ce_loss(outputs[2],label0.long())
...
#         losses3 += loss3
        ...
        '''
        if i_batch%50 == 0:
            print(i_batch,'losses0: {:.3f}'.format(losses0.data),'|','losses1: {:.3f}'.format(losses1.data),'|','losses2: {:.3f}'.format(losses2.data))
...
#             losses3=0
    ...
    torch.save(model.state_dict(),'tip_samed_new.pth')
